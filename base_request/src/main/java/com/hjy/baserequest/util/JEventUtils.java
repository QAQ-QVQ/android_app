...
public class JEventUtils {
     ...
     * @param extra   附加信息
     */
    public static void onCountEvent(String eventId, Map<String, String> extra) {
        Event countEvent = new CountEvent()
                .setEventId(eventId)
                .addExtMap(extra);
        JAnalyticsInterface.onEvent(UtilsManage.getApplication(), countEvent);
    }
     ...
     * 计数事件
     */
    public static void onCountEventJson(Response<String> response, String text) {
        String requestString = "";
        try {
            requestString = response.getRawResponse().toString();
     ...
     * 极光计数事件（接口连接失败使用）
     */
    public static void onCountEventError(Response<String> response, String text) {
        String requestString = "";
        try {
            requestString = response.getRawResponse().toString();
            if (StringUtils.isTrimEmpty(requestString))
        ...
        Map<String, String> extraMap = new HashMap();
        extraMap.put("接口协议", text + "连接失败:" + requestString);
        JEventUtils.onCountEvent("接口连接失败", extraMap);
    }

    /**
     * 极光计数事件（接口返回json数据解析错误使用）
     */
    public static void onCountEventJsonError(Response<String> response, String text) {
        String requestString = "";
        try {
            requestString = response.getRawResponse().toString();
            if (StringUtils.isTrimEmpty(requestString))
        ...
        Map<String, String> extraMap = new HashMap();
        extraMap.put("接口协议及返回json", text + "接口:" + requestString + "\n返回json：" + response.body());
        JEventUtils.onCountEvent("json数据格式错误", extraMap);
    }

    /**
     * 打印完成请求链接
     */
    public static void logRequest(Response<String> response, String text) {
        String requestString = "";
        try {
            requestString = response.getRawResponse().toString();
            if (StringUtils.isTrimEmpty(requestString))
